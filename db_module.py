...
def delRecord(database, contactid):
    conn = sqlite3.connect(database['dbname'])
    cur = conn.cursor()
...
def updateRec(database, _fields, _fieldata):
    conn = sqlite3.connect(database['dbname'])
    cursor = conn.cursor()
    sql = "UPDATE " + database['tblname'] + " SET "
    for f in range(len(_fields) -1):
        sql += _fields[f] + "=?, "
    sql = sql[:-2] + ' WHERE contactid  = ?'
    cursor.execute(sql, _fieldata)
    conn.commit()
    conn.close()
...
def main():
    ...
    whereclause = "Where state = 'va'" # Setting the 'where' (condition) clause variable to only find contacts in Missouri
    getRecords(database,whereclause) # call to the getRecords method sending the database name and a 'where' (condition) clause
    print("-"*25)
    delRecord(database, '2')
    whereclause = ""
    getRecords(database, whereclause)
    print("-"*25)
    fields = ['last', 'first', 'contactid']
    fieldata = ['Nelson', 'Russell M.', 3]
    updateRec(database,fields,fieldata)
    getRecords(database,whereclause)
